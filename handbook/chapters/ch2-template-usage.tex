...
\begin{itemize}
  \isGraduateTF{
    ...
    \item \texttt{5\_resume.tex} 对应早年博士学位论文需要的简历。
    \item \texttt{acknowledgements.tex} 对应致谢；
  }{
...
增添成果需首先在 \texttt{pub.bib} 中添加新的文献条目，然后在 \texttt{4\_pub.tex} 中引用。

如想在盲审模式隐藏自己的名字，那么请参考 pub.bib 中的示例以及注释说明，
或利用 \href{https://bithesis.bitnp.net/faq/pub-author-an.html}{BIThesis 网站的代码生成器}，
向每条文献添加 \texttt{author+an} 字段。
比如，作为张三的你如下设置文献条目，那么你的名字会在盲审模式自动替换为「第一作者」。
...
\isGraduateTF{}{
  \section{提交答辩版、存档版论文}
  完成匿名评阅后，还会再提交答辩版和存档版论文。大部分学院要求这两版论文开头的《原创性声明》和《关于使用授权的声明》应有签名。如果选择电子签名，考虑到签名图片需要调整位置、透明度，用 Word 制作 \texttt{originality.pdf} 再导入 \LaTeX 比较方便，详见 \texttt{main.tex} 中注释；当然打印后签在纸上一般也无问题。

  另外，个别学院要求论文每页有水印，操作方法请参考\href{https://bithesis.bitnp.net/faq/watermark.html}{如何添加「北京理工大学」水印 | BIThesis}。
}
