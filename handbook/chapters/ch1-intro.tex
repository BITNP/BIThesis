...
一般用的标准 \LaTeX{} 发行版就是 \TeX{}Live（macOS 上又名 Mac\TeX{}），简单安装方法见\cref{sec:local-compile}。
如果需要精简安装内容或其它定制，可参考\href{https://bithesis.bitnp.net/guide/getting-started.html}{安装 \LaTeX 环境 | BIThesis}。
...
有关在线编辑平台 Overleaf/TeXPage 的相关内容，请继续阅读\cref{sec:online-local-compare}；
否则，大家可以直接跳转到\cref{sec:using-bithesis}，了解模板的使用方法。
}

\section{本地编译与在线平台，我该使用哪一个？}
\label{sec:online-local-compare}
...
\begin{itemize}[noitemsep]
  ...
  \item \textbf{缺点}
    \begin{itemize}[noitemsep]
      ...
      \item 编译受限，特别是免费时长有限\footnote{2026年3月，\href{https://cn.overleaf.com/blog/changes-to-free-compile-timeout\#:~:text=timeout\%20limit\%20to-,10\%20seconds,-.\%20We\%20made}{免费版 Overleaf 是 10 s}，\href{https://www.texpage.com/zh/pricing/}{免费版 TeXPage 是 30 s}。}，参考文献、图片特别多时，只能\href{https://bithesis.bitnp.net/faq/slow.html}{变通}。
...
\label{sec:overleaf-quirks}

\textit{
\cref{sec:online-compile}介绍了如何从 \href{https://bithesis.bitnp.net}{BIThesis.bitnp.net} 新建项目，那样通常更简单；不过若您已在本地用了模板，想转到 Overleaf，请参考此节。
另外，若想转到 TeXPage，直接按网页提示操作即可；以下特别设置只有 Overleaf 需要。
}
...
\begin{enumerate}
  \item 按网页提示上传文件到 Overleaf，注意\textbf{避免嵌套文件夹}。
    ...
    类似 TeXPage 的\cref{fig:online-compile}，文件 \texttt{main.tex}、文件夹 \texttt{chapters/} 等在根目录，而没有嵌套在 \isGraduateTF{\texttt{graduate-thesis/}}{\texttt{undergraduate-thesis/}} 文件夹中。若您已嵌套，可到左侧文件列表单击再拖动来移动文件。
