...
之后，我们需要将这一文件（\keys{doc} 或 \keys{docx}）保存，\textbf{留作 pandoc 的格式参考。}
...
比如，我们的 LaTeX 文档文件名称为 \keys{main.tex}，参考模板 \keys{Word} 文档名称为 \keys{template.docx}，希望输出名为 \keys{main.docx} 的 Word 文档，我们即可如下组织 pandoc 转换命令：
...
如果你的 LaTeX 文档中包含有参考文献的引用，那么你需要特别明确参考文献 BibTeX 文件，将文件以 \keys{--bibliography={参考文献文件}} 的参数告知 pandoc，从而让 pandoc 正确处理你的参考文献。比如：
