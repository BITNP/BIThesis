...
\ExplSyntaxOn
\luabridge_now:n { require("assets.faq").setup("assets/faq.tex") }
\ExplSyntaxOff

\title{\includegraphics[width=0.3\textwidth]{assets/icon.png}
\\[1cm]
\bfseries 北京理工大学{\LaTeX}学位论文及报告模板 }
\author{北京理工大学网络开拓者协会 BIThesis 开发组 \\ \texttt{bithesis@bitnp.net}} %
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item 项目代码也随之被拆分成了 \BIThesisMacroPackage、
      \href{https://github.com/BITNP/BIThesis-wiki}{\BIThesisWiki} 和 \href{https://github.com/BITNP/BIThesis-scaffold}{\BIThesisScaffold}。
...
\begin{itemize}
  ...
  \item  GitHub 发布：
    ...
    \footnote{最推荐使用此种方式}
  \item  Overleaf/TeXPage 发布：
    \href{https://bithesis.bitnp.net/guide/preface.html#q-bithesis-都包含哪些模板}{Overleaf/TeXPage Templates}
\end{itemize}
其中，CTAN 上发布的是 bithesis 宏包，也就是 |*.cls| 组成的文件，
它们可以通过 \TeX 发行版自带的包管理器 tlmgr 来更新。

GitHub 和 Overleaf/TeXPage 上发布的是 \BIThesisLaTeX 的完整模板，因此想要升级
...
需要注意的是，GitHub 和 Overleaf/TeXPage 的模板中包含了当前版本的 |*.cls| 文件，
...
\item Windows、Linux用户推荐安装 TeX Live 套装，
  并更新宏包（Linux系统由于版权问题，未能预装宋体等 Windows 下的字体，需要手动安装；对于 WSL 用户，可参照\FAQ{word-font}[wsl]直接使用 Windows 下的字体）
...
\end{bitsyntax}
  ...
  在 Windows 和 macOS 中，该字体已经安装；在 Linux 中一般需要用户自行安装（如果你是 WSL 用户，可参照\FAQ{word-font}[wsl]直接使用 Windows 下的字体）。
\end{function}
...
\begin{function}[added=2024-03-22,updated=2026-03-11]{cover/showSpecialTypeBox}
\begin{bitsyntax}[emph={[1]showSpecialTypeBox}]
showSpecialTypeBox = (*false|(true)*)
\end{bitsyntax}

  展示「学生类型」（研究生模板）的那个信息框。|true|（默认）表示始终显示；|false| 表示允许隐藏——至少勾选框中一项时显示，无一勾选时隐藏。

  此选项现在默认为 |true|，但曾经默认为 |false|。最初2024年信息框重要程度较低，可为美观而关闭；后来2026年重要程度变高了，遂改为始终显示。

  只适用于\BIThesisTemplates{GT}，对其它模板无影响。
...
\end{bitsyntax}
  ...
  UDC分类号，用于\BIThesisTemplates{GT}。
\end{function}

\begin{function}[added=2026-03-21]{info/universityCode}
\begin{bitsyntax}[emph={[1]universityCode}]
universityCode = (*\marg{字符串}*)
\end{bitsyntax}

  学校代码，用于\BIThesisTemplates{GT}。
...
\begin{function}[updated=2026-03-21]{info/submissionDate,info/submissionDateEn}
\begin{bitsyntax}[emph={[1]submissionDate,submissionDateEn}]
submissionDate = (*\marg{字符串}*)
submissionDateEn = (*\marg{字符串}*)
\end{bitsyntax}
...
\begin{function}[updated=2026-03-31]{style/head}
...
\end{bitsyntax}
  ...
  \BIThesisTemplates{PT}的默认值含“外文翻译”几字，有的学院要求去掉，这时请自行修改。

  若此选项设置为空，则不仅会清空页眉文字，还会一同删除页眉线。若需保留页眉线但清空文字，可设置此选项为空白字符，例如 |{~}|。
\end{function}
...
\begin{itemize}
  ...
  \item 在发表时间顺序上微调，把个别的提到最前——保留默认的 |true|，同时在 |*.bib| 文件中给个别项加上 |sortkey| 字段。
    ...
    具体例子可参考\FAQ{bib-sort}[sortkey]，详细解释可参考 \href{https://mirrors.ctan.org/macros/latex/contrib/biblatex-contrib/biblatex-gb7714-2015/biblatex-gb7714-2015.pdf}{biblatex-gb7714-2015 宏包手册（中文）}或 \href{https://mirrors.ctan.org/macros/latex/contrib/biblatex/doc/biblatex.pdf}{biblatex 宏包手册（英文）}。
\end{itemize}
...
acknowledgements = (*\marg{键值列表}*)
acknowledgements/(*\meta{key}*) = (*\meta{value}*)
\end{bitsyntax}

 该选项包含许多子项目，用于调整其他选项。具体内容见下：
\end{function}

\begin{function}[added=2025-05-15]{acknowledgements/showTitleInPeerReview}
\begin{bitsyntax}[emph={[1]showTitleInPeerReview}]
showTitleInPeerReview = (*false|true*)
\end{bitsyntax}
...
\begin{itemize}
  \item （本科模板默认）若设为 |false|，盲审模式下直接删除致谢。
  \item （硕博模板默认）若设为 |true|，盲审模式下保留致谢题目，只是隐去致谢内容。
\end{itemize}
...
\end{bitsyntax}
  ...
  在 Windows 和 macOS 中，该字体已经安装；在 Linux 中需要用户自行安装（如果你是 WSL 用户，可参照\FAQ{word-font}[wsl]直接使用 Windows 下的字体）。
\end{function}
...
\begin{function}[added=2023-04-22,updated=2023-05-09]{misc/autoref/algo, misc/autoref/them, misc/autoref/lem,
 misc/autoref/prop, misc/autoref/cor, misc/autoref/axi, misc/autoref/assm, misc/autoref/defn, misc/autoref/conj,
...
const/autoref/lem,const/autoref/prop,const/autoref/cor,const/autoref/axi,const/autoref/assm,
...
\begin{function}[updated=2026-03-24]{\MakeTitle}

  绘制中英文题名页，用于\BIThesisTemplates{GT}。
...
\end{bitsyntax}
  ...
  另可参考\FAQ{symbols-bracket}、\FAQ{symbols-variant}。
...
\begin{function}[updated=2023-03-05]{algo,them,lem,prop,cor,axi,assm,defn,conj,exmp,case,rem}
...
\end{bitsyntax}
  ...
  其中提供了算法环境 |algo|，但模板也适配了一些更专业的宏包，请参考\FAQ{algorithm}。
\end{function}
...
\subfloat[][plain样式]{
  ...
  \toprule
  \textbf{名称} & algo & them & lem & prop & cor & axi & assm \\ \midrule
  \textbf{全称} & algorithm & theorem & lemma & proposition & corollary & axiom & assumption \\
  \textbf{含义} & 算法        & 定理      & 引理    & 命题          & 推论        & 公理    & 假设    \\
  \textbf{样式} & \multicolumn{7}{c}{\textbf{定理2.1.} \textit{定理内容……}}       \\ \bottomrule
  \end{tabular}
}
...
\begin{function}[updated=2026-03-24,label=env.acknowledgements]{acknowledgements}
\begin{bitsyntax}[emph={[1]acknowledgements}]
...
\end{bitsyntax}
  ...
  盲审模式下，致谢的内容始终隐藏，而标题显示还是隐藏由 \hyperref[doc/function//acknowledgements/showTitleInPeerReview]{|acknowledgements/showTitleInPeerReview| 选项}决定。
\end{function}
